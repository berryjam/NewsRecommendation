...
/**
 * CrawlController collects all the classes which cooperate to perform a crawl
 * and provides a high-level interface to the running crawl.
 *
 * As the "global context" for a crawl, subcomponents will often reach each
 * other through the CrawlController.
 *
 * @contributor gojomo
 */
public class CrawlController implements Serializable, Lifecycle,
		ApplicationContextAware, Checkpointable {
	 ...
	 */
	protected ConfigPath scratchDir = new ConfigPath("scratch subdirectory",
			"scratch");
	...
	public void start() {
		...
		// force creation of DNS Cache now -- avoids CacheCleaner in toe-threads
		// group
	...
	protected void sendCrawlStateChangeEvent(State newState, CrawlStatus status) {
		...
		this.state = newState;
		CrawlStateEvent event = new CrawlStateEvent(this, newState,
	...
	protected void completeStop() {
		...
		// CrawlJob needs to be sure all beans have received FINISHED signal
		// before teardown
	...
	/**
	 * Is this crawl actively able/trying to crawl? Includes both states RUNNING
	 * and EMPTY.
	 * 
	 * @return
	 */
	...
	/**
	 * Evaluate if the crawl should stop because it is finished, without
	 * actually stopping the crawl.
	 * 
	 ...
	 * 
	 * @param msg
	 *            Message to write the progress statistics log.
	 */
	...
	/**
	 * Receive notification from the frontier, in the frontier's own manager
	 * thread, that the frontier has reached a new state.
	 * 
	 * @param reachedState
	 *            the state the frontier has reached
	 */
